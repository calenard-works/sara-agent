...
3. **Project Policy** (Persistent) - Stored permissions in `.sara/config.json`
...
- **Internal config**: Auto-allowed for `.sara/` directory operations
...
- **Project grants**: `.sara/config.json`, persists across sessions, used for "Don't ask again" approvals
...
Check Approval Mode (autoEdit/yolo = auto-approve)
     ↓
Check .sara/ directory access (auto-allowed)
     ↓
...
sara config set approvalMode default

# Auto-approve file write operations
sara config set approvalMode autoEdit

# Auto-approve all operations (FS + bash)
sara config set approvalMode yolo
```
