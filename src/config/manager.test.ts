...
// Test utilities
const testConfigDir = path.join(os.tmpdir(), "sara-test-config");

beforeEach(() => {
  // Set up clean test environment
  delete process.env.SARA_API_KEY;
  delete process.env.SARA_BASE_URL;
  delete process.env.SARA_MODEL;
  delete process.env.SARA_PLAN_MODEL;
...
describe("ConfigManager", () => {
  describe("load", () => {
    ...
    it("should auto-detect DeepSeek API key", () => {
      ...
      // Mock the global config path to use test directory (clean state)
      const originalHomedir = os.homedir;
      os.homedir = () => testConfigDir;
  ...
  describe("set", () => {
    it("should set configuration values", () => {
      ...
      try {
        ...
        const configPath = path.join(testConfigDir, ".sara", "config.json");
