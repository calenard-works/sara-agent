 ...
 *
 * Priority:
 * 1. SARA_* variables (explicit user configuration)
...
function loadFromEnvironment(): {
  ...
  // Check for explicit SARA_* configuration first (highest priority)
  const customApiKey = process.env.SARA_API_KEY;
  const customBaseURL = process.env.SARA_BASE_URL;
  const customModel = process.env.SARA_MODEL;
  const customPlanModel = process.env.SARA_PLAN_MODEL;
  ...
  if (opencodeKey?.trim()) {
    return {
      baseURL: API_PRESETS.opencode.baseURL,
      model: API_PRESETS.opencode.model,
      planModel: API_PRESETS.opencode.planModel,
...
function readGlobalConfig(): GlobalConfig {
  ...
  try {
    ...
    if (typeof parsed.provider === "string" && parsed.provider.trim()) {
      config.provider = parsed.provider.trim();
    }
...
export class ConfigManager {
  ...
  static load(): EffectiveConfig {
    ...
    // Check if user has explicitly selected a provider via /login
    // If so, use that provider's config and ignore env-based auto-detection
    const hasExplicitProvider = !!globalConfig.provider;
    const explicitApiKey = hasExplicitProvider
      ? globalConfig.apiKeys?.[globalConfig.provider!]
      : undefined;

    // Resolve LLM configuration
    // When explicit provider is set, env-based baseURL/model are ignored
    const resolvedBaseURL = resolveWithSources({
      envValue: hasExplicitProvider ? undefined : envConfig?.baseURL,
      globalValue: globalConfig.llm?.baseURL,
      defaultValue: DEFAULT_CONFIG.llm.baseURL,
    });

    const resolvedModel = resolveWithSources({
      envValue: hasExplicitProvider ? undefined : envConfig?.model,
      globalValue: globalConfig.llm?.model,
      defaultValue: DEFAULT_CONFIG.llm.model,
    });

    const resolvedPlanModel = resolveWithSources({
      envValue: hasExplicitProvider ? undefined : envConfig?.planModel,
      globalValue: globalConfig.llm?.planModel,
      defaultValue: DEFAULT_CONFIG.llm.planModel,
    });

    // API key: explicit provider key > env var > empty
    const resolvedApiKey = resolveWithSources({
      envValue: explicitApiKey ?? envConfig?.apiKey,
  ...
  /**
   * Set current provider ID and update LLM config accordingly
   */
  static setProvider(providerId: string): void {
    const config = readGlobalConfig();
