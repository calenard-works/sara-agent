...
export async function* streamChatCompletion(
  ...
  for await (const chunk of stream) {
    ...
    // CRITICAL BUG FIX: Ensure isComplete: true is yielded ONLY ONCE.
    // The old condition `content || finishReason !== null || parsedToolCalls.length > 0`
    // caused EVERY chunk after finishReason was set to yield with isComplete: true again.
    // This made the UI reducer append duplicate assistant messages.
    const isStreamNowComplete = finishReason !== null && !hasYieldedComplete;
    const hasContentOrToolCalls = content || parsedToolCalls.length > 0;

    if (isStreamNowComplete) {
      hasYieldedComplete = true;
      yield {
        completeMessage,
        parsedToolCalls:
    ...
    } else if (hasContentOrToolCalls && !hasYieldedComplete) {
      yield {
        completeMessage,
        parsedToolCalls:
          parsedToolCalls.length > 0 ? parsedToolCalls : undefined,
