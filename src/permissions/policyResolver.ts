 ...
 * - Project policy (persistent, stored in .sara/config.json)
 ...
 * 2. Check internal .sara directory access
...
export function checkFsPermission(
  ...
  // Auto-allow operations on internal configuration files within .sara directory
  // This allows tools like todoWrite to work without permission prompts
  const saraDir = cwd + "/.sara";
  if (isPathUnderPrefix(targetPath, saraDir)) {
    return { ok: true };
  }
