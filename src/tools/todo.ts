...
function todosPath(cwd: string, sessionId: string): string {
  return path.join(cwd, ".sara", "sessions", sessionId, "todos.json");
}
...
export const TodoWriteTool: Tool<z.infer<typeof TodoWriteInput>, TodoResult> = {
  ...
  // Note: readonly=true means it doesn't modify user files directly. It only writes to .sara/sessions/ which are internal session files, not user project files.
  readonly: true,
  ...
  async execute(input, context) {
    ...
    // TodoWriteTool operates on .sara directory which is auto-allowed
