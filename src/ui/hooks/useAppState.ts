...
export type AppState = {
  ...
  /** Interactive mode for /login, /model commands */
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "COMPLETE_LLM_MESSAGE": {
      ...
      if (action.message.message.role === "assistant") {
        ...
        if (
          ...
          // CRITICAL BUG FIX: Handle duplicate COMPLETE_LLM_MESSAGE.
          // If the last message is already a complete assistant message from the same
          // LLM turn, update it in-place instead of appending. This prevents duplicates
          // when the LLM provider sends multiple finish_reason chunks.
          last &&
          last.kind === "api" &&
    ...
    case "SET_INTERACTIVE_MODE":
      return {
        ...state,
        interactiveMode: action.mode,
      };

    case "CLEAR_INTERACTIVE_MODE":
      return {
        ...state,
...
export function useAppState(initialApprovalMode: ApprovalMode = "default") {
  ...
  const useDebugData = process.env.SARA_DEBUG === "true";
  ...
  const actions = {
    ...
    setInteractiveMode: useCallback((mode: InteractiveMode) => {
      dispatch({ type: "SET_INTERACTIVE_MODE", mode });
    }, []),
